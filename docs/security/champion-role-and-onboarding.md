...
- [ ] **Confirm the template actually shows up.** Go to the repo's Issues tab → New Issue, and confirm "Design Review" appears as a selectable template. If it doesn't, the template file has likely drifted out of `.github/ISSUE_TEMPLATE/` (the only folder GitHub's issue picker recognizes) — check before assuming the process is ready to run. This exact failure happened during the Day 3 pilot: the file was briefly in `docs/security/` instead, and no template appeared when it came time to open a real issue.
